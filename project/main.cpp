...
void updateObjectProperties(int objectIndex)
{
    string urlList[5] = {"./models/teapot.obj", "./models/rock.obj", "./models/heart.obj", "./models/star.obj", "./models/teapot.obj"};

    // load an object file
    bool fileload;
    fileload = object->loadFromFile(urlList[objectIndex]);
    if (!fileload)
    {
        #if defined(_MSVC)
        fileload = object->loadFromFile(urlList[objectIndex]);
    ...
    object->setUseMaterial(true, true);
    ...
    object->computeAllEdges(0);    // set normal properties for display
    ...
    object->setLocalPos(-1 * object->getBoundaryCenter());

    //SPECIFIC STUFF FOR OBJECT

    object->setStiffness(0.7 * maxStiffness, true);
    object->setFriction(0.0, 0.3, true);
}
...
void keyCallback(GLFWwindow* a_window, int a_key, int a_scancode, int a_action, int a_mods)
{
    ...
    {
        updateObjectProperties(0);
    }
    else if (a_key == GLFW_KEY_1)
    {
        updateObjectProperties(1);
    }
    else if (a_key == GLFW_KEY_2)
    {
        updateObjectProperties(2);
    }

    else if (a_key == GLFW_KEY_3)
    {
        updateObjectProperties(3);
    }
    else if (a_key == GLFW_KEY_4)
    {
        updateObjectProperties(4);
    }
}
