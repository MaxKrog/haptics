...
void HapticWall::initialize(cWorld* world, cCamera* camera)
{
    ...
    cShapeLine *myWall = new cShapeLine(cVector3d(0,0.02,1), cVector3d(0,0.02,-1));

    world->addChild(myWall);
}
...
void HapticWall::updateHaptics(cGenericHapticDevice* hapticDevice, double timeStep, double totalTime)
{
    ...
    if(newPosition.y() > 0.01){
        double howFarIn;
        howFarIn = newPosition.y() - 0.01;

        force.y(howFarIn * -200);
